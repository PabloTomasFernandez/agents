...
class Me:
    ...
    def __init__(self):
        ...
        self.name = "Pablo Tomás Fernández"
    ...
    def system_prompt(self):
        ...
        system_prompt += f"No inventes datos que no the he dado."
        return system_prompt
    
    def chat(self, message, history):
        # 1. Limpiar el historial para dejar solo las claves que la API de Groq/OpenAI acepta
        clean_history = []
        for msg in history:
            clean_msg = {k: v for k, v in msg.items() if k in ["role", "content", "name", "tool_calls", "tool_call_id"]}
            clean_history.append(clean_msg)

        # 2. Construir la lista de mensajes con el historial limpio
        ...
        while not done:
            ...
            if response.choices[0].finish_reason=="tool_calls":
                # Al recibir la respuesta con la llamada a la herramienta, la convertimos a diccionario 
                # (o usamos su formato compatible) para evitar problemas al re-enviarla
